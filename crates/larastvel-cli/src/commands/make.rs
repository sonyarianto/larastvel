...
pub fn make_command(name: &str) {
    ...
    let command_content = format!(
        r#"use larastvel_core::console::{{Command, ConsoleError}};
use larastvel_core::foundation::Application;

/// {name}
#[derive(Debug)]
#[command("{name}", description = "Description of the {name} command")]
pub struct {struct_name};

impl {struct_name} {{
    pub fn run(&self, _app: &Application, _args: &[String]) -> Result<(), ConsoleError> {{
...
pub fn make_policy(name: &str) {
    ...
    let policy_content = format!(
        r#"use larastvel_core::auth::{{AuthenticatedUser, GateCheck}};

/// Policy for managing {resource} authorization.
#[derive(Debug)]
#[policy("{resource}")]
pub struct {name};

impl {name} {{
    /// Define authorization checks for this policy.
    /// Called by the `Policy` trait implementation generated by `#[policy]`.
...
impl {name} {{
    pub async fn seed(conn: &DbConn) -> Result<(), Box<dyn std::error::Error>> {{
...
pub fn make_factory(name: &str) {
    ...
    let factory_content = format!(
        r#"use larastvel_core::models::factory::Faker;
...
impl {struct_name} {{
    /// Define default attribute values for the model.
    /// The `#[factory]` macro generates the `ModelFactory` impl from this method.
    pub fn define() -> crate::models::{snake}::ActiveModel {{
...
pub fn make_resource(name: &str) {
    let resources_dir = std::path::Path::new("src/resources");
    std::fs::create_dir_all(resources_dir).unwrap();

    let snake_name = to_snake_case(name);
    let struct_name = if name.ends_with("Resource") {
        name.to_string()
    } else {
    ...
    let resource_content = format!(
        r#"use larastvel_core::api_resource;
use serde_json::Value;

/// API resource for {name}.
#[derive(Debug)]
#[api_resource(crate::models::{snake}::Model)]
pub struct {struct_name};

impl {struct_name} {{
    /// Transform a model into its API representation.
    pub fn to_array(model: &crate::models::{snake}::Model) -> Value {{
        // TODO: Define API output fields
        // Example:
        // serde_json::json!({{
        //     "id": model.id,
        //     "name": model.name,
...
"#,
    ...
    let file_path = resources_dir.join(format!("{}.rs", snake_name));
    if file_path.exists() {
        eprintln!(
            "{}",
            format!(
                "Error: Resource '{}' already exists at '{}'.",
                name,
                file_path.display()
            )
            .red()
    ...
    std::fs::write(&file_path, resource_content).unwrap();
    ...
    println!(
        "{}",
        "  Implement to_array() to define the API output fields.".dimmed()
    );
}

pub fn make_provider(name: &str) {
    let providers_dir = std::path::Path::new("src/providers");
    std::fs::create_dir_all(providers_dir).unwrap();

    let snake_name = to_snake_case(name);
    let struct_name = if name.ends_with("Provider") {
        name.to_string()
    } else {
    ...
    let provider_content = format!(
        r#"use larastvel_core::foundation::Application;
...
impl {struct_name} {{
    ...
    /// Called automatically by the `#[provider]` macro.
    pub fn register_services(&self, _app: &Application) {{
        // TODO: Bind services, register policies, etc.
        // Example:
        // _app.bind::<dyn MyService>(MyServiceImpl::new());
    }}
}}
"#,
        struct_name = struct_name,
    ...
    let file_path = providers_dir.join(format!("{}.rs", snake_name));
    if file_path.exists() {
        eprintln!(
            "{}",
            format!(
                "Error: Provider '{}' already exists at '{}'.",
                name,
                file_path.display()
            )
            .red()
    ...
    std::fs::write(&file_path, provider_content).unwrap();
